...
<?php echo $_GET["check"]; 
  ...
  if ($_GET["check"] = "OK") {
	  $put = pg_execute($link, "UPDATE public.check_ok SET where check_ok = 'OK' where check_result = 'OPT';");
  }elseif ($_GET["check"] = "FAILED") {
	  $put = pg_execute($link, "UPDATE public.check_ok SET where check_ok = 'FAILED' where check_result = 'OPT';");
  }
